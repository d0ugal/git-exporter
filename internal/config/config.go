...
// Git repository configuration requires the yaml file (no env var support for repos).
...
// applyEnvVars overlays Git-exporter environment variables onto cfg.
// Only variables that are set (non-empty) are applied.
// Note: git repository configuration (paths, discovery) requires a yaml file.
